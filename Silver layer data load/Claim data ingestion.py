...
df = spark.sql("SELECT c.claim_id, c.claim_amount, c.LastUpdatedTimeStamp, c.policy_id, c.claim_status, to_date(date_format(c.date_of_claim, 'MM-dd-yyyy'), 'MM-dd-yyyy') as date_of_claim from bronze.claim c INNER JOIN bronze.policy p ON c.policy_id = p.policy_id  WHERE c.claim_amount  is not null and c.claim_id is not null and c.policy_id is not null and c.claim_status is not null and c.LastUpdatedTimeStamp is not null and c.merge_flag = FALSE and c.claim_amount >0")
...
# MAGIC %sql
# MAGIC MERGE INTO silver.claim AS T USING clean_claim AS S ON 
