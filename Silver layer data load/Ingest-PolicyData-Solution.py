...
df = spark.sql(" SELECT p.* FROM bronze.policy p INNER JOIN  bronze.customer c  ON p.customer_id = c.customer_id  where p.customer_id is not null and p.policy_id is not null and p.merge_flag = FALSE and p.premium >0 and  p.coverage_amount >0 AND p.end_date > p.start_date ")
display(df)
...
# MAGIC %sql
# MAGIC UPDATE bronze.policy SET merge_flag = True WHERE merge_flag = FALSE
